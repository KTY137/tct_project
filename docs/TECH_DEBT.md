...
| ANNOYANCE | Config validator `_KNOWN_KEYS` gap: `controller/config_validator.py` (~L34-79) has no coverage for 6 `devices.yaml` sections — `camera` (9 keys), `laser` (3), `slow_control` (2), `influx` (5), `output` (2), `charge_calibration` (6). Typos in these sections are silently ignored instead of warned. Low urgency; add the key sets, don't change existing pass behavior. | `TCT_app/controller/config_validator.py` | Paul | 2026-07-06 |
