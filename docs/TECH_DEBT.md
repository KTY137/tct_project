...
| ANNOYANCE | Clean unipolar 0→+V trigger path exists in the driver (`set_levels()` + `level_low_V`/`level_high_V` via `:VOLTage:HIGH/:LOW`) but is NOT reachable from `devices.yaml`: add the two keys to the `WaveformGenerator(...)` construction in `device_manager.py` and to `_KNOWN_KEYS['waveform_generator']`. Bipolar (safe per manual) stays the default; wire only if the user wants config-driven unipolar. | `TCT_app/controller/device_manager.py` | Abel/Adam | 2026-07-07 |
