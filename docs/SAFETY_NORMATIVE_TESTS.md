...
| `test_sequencer_panel.py` :: subset — `test_abort_button_calls_abort_sequence`, `test_on_sequence_active_locks_and_unlocks_manual_danger_panels`, `test_manual_danger_reenables_after_failure_path`, `test_manual_pause_during_sequence_no_dialog_notify_and_abort`, `test_non_sequence_manual_pause_still_shows_dialog_resume_and_abort`, `test_manual_pause_during_real_sequence_aborts_fail_safe_and_parks_hv` | Sequencer panel safety half (retained command/host residue): abort stays live while a sequence runs; sequence-active locks the manual danger panels and re-enables after failure; a MANUAL_PAUSE encountered during a real sequence aborts FAIL-SAFE and parks HV (no interactive dialog inside an autonomous queue). | gate / HV / fail-safe | C | **GUI-half-rehost** | The pause-rejection *logic* is already pinned in bucket A (`test_sequencer.py`); these rows pin the retained command/host behavior. **[U1.3 host update, 2026-07-15: the two (a)-class MODEL rows (`test_arm_text_contains_every_routine_hv_and_travel`, `test_queue_edit_rederives_envelope_no_stale`) were reclaimed into `test_sequencer_viewmodel.py` (row below) — assertions moved host verbatim, never weakened; `docs/design/u1_staging.md` §5.5 + §1.3 reclaim order.]** NO dedup with the danger-gate suites during U1 (Rulings Q6) — the lock/unlock ROUND-TRIP asserted here is a different thing from refusal-under-declining-gate. |
| `test_sequencer_viewmodel.py` :: subset — `test_arm_text_contains_every_routine_hv_and_travel`, `test_queue_edit_rederives_envelope_no_stale` | Sequencer queue-model safety half (informed consent + no-stale-envelope): the combined-envelope summary names EVERY queued routine's HV + travel extremes (informed consent), and a queue edit re-derives it from scratch so a stale envelope can never be armed. Now asserted against the read-only `SequencerQueueViewModel` (fed by a real `SequenceCoordinator` + `FakeScanCoordinator`), with the panel's command/host residue kept separate. | gate / HV / fail-safe | D | **GUI-half-rehost** | Reclaimed C→D from `test_sequencer_panel.py` in U1.3 (`docs/design/u1_staging.md` §5.5; §1.3's reclaim order) — the assertion survives verbatim, re-hosted against the viewmodel surface (host update per the Maintenance rule). The same new suite also carries the replicated standing-law pair (`test_read_only_no_command_surface`, `test_owns_no_timer_no_thread`, Ruling Q3). |
| `test_qt_danger_gate.py` :: QtDangerGate cluster — 5 normative (`test_qt_danger_gate_confirms_true_on_gui_thread`, `test_qt_danger_gate_denies_false_on_gui_thread`, `test_qt_danger_gate_confirm_from_worker_thread`, `test_qt_danger_gate_timeout_denies`, `test_qt_danger_gate_no_stray_dialog_after_shutdown`) + 4 supporting (`…timeout_then_pump_shows_no_stray_dialog`, `…dialog_exception_releases_worker_as_deny`, `…shutdown_denies_pending_and_future`, `…abort_denies_pending_but_stays_usable`) = **9 total** | The `QtDangerGate` contract: confirm/deny resolve on the GUI thread, a worker-thread confirm marshals safely, an unanswered dialog **times out to DENY** (fail-closed), and no stray dialog fires after shutdown; the 4 supporting rows extend the stray-dialog/exception/abort-usability guards. | gate / fail-safe | B | **GUI-half-rehost** | The QtDangerGate modal is on the NEVER-migrates list — these assertions survive verbatim against the retained QWidget gate. RULED (Rulings Q4): the cluster was carved out of `test_planner_panel.py` into `tests/test_qt_danger_gate.py` (PRECONDITION of U1, beat **U1.0**, byte-preserving move). **COUNT DRIFT (open Q3, flagged for Mary ratification):** Ruling Q4 says "5-test cluster"; HEAD carried **9** `test_qt_danger_gate_*` tests, all moved together (5 normative + 4 supporting, exactly as "whole file rides bucket A" supporting rows). Rest of the former host file (drag/drop, template plumbing) is not normative. |
...
- Manifest files: **45** (byte-identical **24** · GUI-half-rehost **19** ·
  ...
  tests covered: **~465** (whole-file rows counted at their full collected
...
4. **QtDangerGate carve-out (v0.1 Q4): APPROVED and hardened.** Carving the
   ...
   widget-light). **[U1.0 execution note, 2026-07-15: HEAD carried 9
   `test_qt_danger_gate_*` tests, not 5 — the cluster grew after this ruling.
   All 9 were moved byte-preserving (5 normative + 4 supporting); the manifest
   row and `docs/test_bucket_map.md` were updated accordingly. The 5→9 count
   drift is open **Q3** (§8 of `docs/design/u1_staging.md`) for Mary to
   ratify at the U1.0 review; the §Counts aggregate below is left unrecomputed
