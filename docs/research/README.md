...
| 2026-07-13 | `scpi_capability_discovery.md` | SCPI capability query patterns | VISA/SCPI discovery of oscilloscope channel count, averaging limits, trigger modes; live-verified on TBS1052C per manual 077-1691; sourced from official reference. |
| 2026-07-13 | `pi_gcs_stop_semantics.md` | PI GCS 2.0 stop (#24) semantics | PI C-663 `StopAll` (#24) is a lock-free single-character real-time byte, completes in one exchange (sub-10 ms latency), verified against GCS reference manual; complements guarded_exchange pattern. |
