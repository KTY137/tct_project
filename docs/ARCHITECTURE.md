...
- `run_state_viewmodel.py` — Read-only run-state facade (2026-07-11 slice 1, commit 713eeae): `RunStateViewModel` exposes current step index, total steps, run-state enum, pause/resume/abort affordances via no-controller-refs `changed` notifier, fed by shared 1 Hz `_light_timer` + coordinator signals. **Owns the single ETA derivation** (elapsed time + plan estimate → remaining time + formatted `hh:mm:ss`). Exposed to QML as `'runState'` context property (only notifies via `changed`, no direct signal binding). Built before `CalibrationPanel` in `_build_central()`. Teardown in viewmodel-release block ensures Qt lifecycle safety.
- `scan_map_viewmodel.py` (U1.1) — Read-only scan-map state for viewer integration (U1 reclaim from `test_scan_map_view.py` (a)-class tests, commit a88b823): `ScanMapViewModel` owns the points store (update/batch-load normalization, rounding-collision + duplicate/missing accounting), quantity selection, grid derivation via composed `analysis.scan_grid.points_to_grid`, NaN autoscale levels, cursor-readout derivation (`valueAt`), and CSV export. No controller/coordinator refs. Composed by `scan_map_view.py` panel.
- `scan_viewer_viewmodel.py` (U1.2) — Read-only scan-viewer state for live run monitoring (U1 reclaim from `test_scan_viewer_panel.py` (a)-class tests, commit 62cec74): `ScanViewerViewModel` composes `RunStateViewModel` (single run-state truth) + adds viewer-specific state: progress-tile staleness (armed-but-stale until first progress), current-position text, manual-pause warn, z-focus curve data model (point accumulation, reset-on-start), open-in-analysis eligibility (terminal AND runPath including path-set-after-finish), run-path invalidation on new run. No direct commands. Receiver for coordinator signals (on_scan_started/on_progress/on_point_done/on_scan_finished/on_error + viewer-local on_z_focus_pt/on_z_focus_done/set_last_run_path/on_manual_pause). Composed by `scan_viewer_panel.py`.
- `sequencer_viewmodel.py` (U1.3) — Read-only sequencer queue state for display (U1 reclaim from `test_sequencer_panel.py` (a)-class tests, commit a5bacf2): `SequencerQueueViewModel` mirrors the active queue (entry list, run state, envelope summary) from `SequenceCoordinator`, with no command surface or thread ownership. Subtree: `test_arm_text_contains_every_routine_hv_and_travel`, `test_queue_edit_rederives_envelope_no_stale` pinned against the viewmodel surface (assertions moved host from panel residue, never weakened). Composed by `sequencer_panel.py` for command/safety host separation.
...
**QML component kit (U2.1, 2026-07-15):**

- `qml/kit/` — Core QML material system (U2.1, commit `1ab0085`): `Surface.qml` (rung ladder, tier resolution, state table, hazard construction-throw; the one material: no inline paint), `LivingGround.qml` (animated ambient ground, band law, run-active clamp, bake cadence), `FocusRing.qml` (focus indicator), `KitEnv.qml` (scene-state singleton: tier/runActive/calmPolicy/dead-zone registry), `FrostBake.qml` + `FrostCrop.qml` (internal: the application's ONE MultiEffect blur + per-pane crop samplers), `KitAssets.qml` + `assets/` (12 GENERATED 9-patch/wash PNGs baked from `gui/style.py` tokens; regenerate via `scripts/gen_shadow_assets.py` after token changes; PNGs ride Git LFS). The component library on top (PanelHeader, StatusPill, Card, MetricTile, MetricGrid, CollapsibleCard, ActionBar, EmptyState, FormRow, FigureCard) is **U2.2, in flight — indexed when it lands**. **Invariants:** components never own paint (Surface does); exactly one MultiEffect app-wide; tokens only via `gui/qml_theme.py` TOKEN_MAP (inline-hex guard covers kit QML); calm policy is ONE switch (panel default, global fallback); types are exported via `qmldir` (directory import `"kit"`, no module URI). Test coverage: `tests/test_qml_kit_surface.py` (27 tests, kit material laws).
...
- `gen_shadow_assets.py` (NEW, U2.1) — QML kit asset generation: 9-patch shadow/halo/wash PNG assets (12 total, light+dark variants baked from accent/shadow tokens in `gui/style.py`; asserts the band-law tint budget at bake time). Run after any design-token change to `style.py` before committing `gui/qml/kit/assets/`; a manifest drift-guard test fails loudly until regenerated. Produces LFS-tracked PNGs (numpy-based, no GUI).
...
- 2026-07-15 — **feat(gui): U2.1 QML component kit core (1ab0085).** NEW `gui/qml/kit/` module: Surface.qml (rung ladder, tier resolution, hazard construction-throw), LivingGround.qml (animated ground, band law, run-active clamp), FocusRing.qml, KitEnv.qml (scene-state singleton), FrostBake.qml + FrostCrop.qml (the ONE app-wide MultiEffect + per-pane crop), KitAssets.qml + 12 GENERATED 9-patch/wash PNGs (LFS-tracked, regenerate via scripts/gen_shadow_assets.py). NEW `tests/test_qml_kit_surface.py` (27 tests, material laws). Invariants: components never own paint (Surface does), one MultiEffect app-wide, tokens via TOKEN_MAP (no inline hex), calm policy = one switch.

- 2026-07-15 — **U2 in flight:** U2.2 component library (PanelHeader/StatusPill/Card/MetricTile/MetricGrid/CollapsibleCard/ActionBar/EmptyState/FormRow/FigureCard) building — indexed when landed. Overlay spike FAILED on quiet machine (scene 60→23 fps with raster island in one top-level; control clean) → mitigation matrix landed `ce46074`, windowed matrix run pending; U2.4 host code gated on its verdict. DECISIONS same day: kit_spec_v1 SIGNED (Kaya); measurement-B thresholds RATIFIED unchanged; ruling 10 (ONE shared frost bake; §2.2 card row amended).
...
- 2026-07-15 — **feat(gui): U1.2 ScanViewerViewModel extraction + RunStateViewModel ETA consolidation (62cec74).** NEW `gui/scan_viewer_viewmodel.py`: composes RunStateViewModel (run-state truth, now owns ETA derivation) + viewer staleness/position/z-focus/path state. C→B: test_scan_viewer_panel.py reclassified; 15 (a)-class tests reclaimed into test_scan_viewer_viewmodel.py; (b) residue byte-untouched, green. New D row: test_scan_viewer_viewmodel.py.

- 2026-07-15 — **feat(gui): U1.3 SequencerQueueViewModel extraction (a5bacf2).** NEW `gui/sequencer_viewmodel.py`: read-only queue/run/envelope mirror, no command surface. C→B: test_sequencer_panel.py reclassified; 6 (a)-class tests reclaimed, including 2 manifest-listed rows (test_arm_text_contains_every_routine_hv_and_travel, test_queue_edit_rederives_envelope_no_stale) now hosted in test_sequencer_viewmodel.py; (b/c) residue byte-untouched, green.
