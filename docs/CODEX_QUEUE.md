...
**Status: DONE - Logged v0.2 capability-model review blockers.** - Effort: M - Source: Adam, 2026-07-13 evening (free-lane parallel value)
...
**Codex findings (2026-07-13):**
- Files touched: `docs/CODEX_QUEUE.md` only. No code, drivers, tests, or `docs/CAPABILITY_MODEL.md` were modified by this lane.
- Review target note: `git log --oneline -5 -- docs/CAPABILITY_MODEL.md` still shows only `a6d58e0` (`v0.1-draft`), but the working tree changed during review to `docs/CAPABILITY_MODEL.md` v0.2. Findings below are against the current working-tree v0.2 because that is the live "current" spec Adam asked to re-review.
- BLOCKER - `docs/CAPABILITY_MODEL.md:CapabilityDescriptor` / `docs/CAPABILITY_MODEL.md:section 6.1 Transport reservation` contradict the new transport split. v0.2 adds `transport_id` and the field table says it is the lock-resolution/deadlock-ordering key, but section 6.1 still says grouped reservations sort by `device_id` and that pass-through adapters declare the underlying lock via `device_id`; section 10 also omits `transport_id` from `swept/` attrs. Failure case: a scope-backed `intensity.amplitude` needs `device_id="intensity_monitor"` for identity/provenance and `transport_id="scope"` for locking; following section 6.1 makes either lookup/provenance wrong or locking wrong. Amendment: make section 6.1 use `transport_id` everywhere, define the exact lock resolver (`DeviceManager`/registry API), include `transport_id` in `swept/` attrs, and add a pass-through ScopeChannelMonitor test.
- BLOCKER - `docs/CAPABILITY_MODEL.md:section 10 HDF5 provenance` loses command events by forcing `swept/` rows to align only with `/points` and saying no `/points` means no `swept/`. `TCT_app/controller/scan_controller.py:_apply_wavegen_settings` records acquire-attempt commands before acquisition/save, while `TCT_app/SCAN_DATA_FORMAT.md` has non-`/points` scan tables (`/voltage_scan`, `/z_focus`). Failure case: a wavegen setter succeeds, the acquire or save fails, and the future `swept/` table has no row for the real command; likewise a voltage scan cannot record `bias.voltage` under `swept/` without inventing a `/points` row. Amendment: add an apply/attempt-indexed command-event dataset or status row, and explicitly define alignment for voltage/z-focus tables before freezing DA1.
- MAJOR - `docs/CAPABILITY_MODEL.md:sections 5.1/5.4` and `sections 7.3/11.1` disagree on intensity IDs. v0.2 correctly decides scalar `intensity.amplitude` and `intensity.charge` wrapping `IntensityMonitorBase.get_amplitude` / `get_charge`, but class assignments and the adapter table still publish `intensity.reading` wrapping composite `IntensityMonitorBase.read`. Failure case: D1 either freezes the stale `intensity.reading` permanent id or shoves `IntensityReading`'s amplitude, charge, waveform arrays, and saturation flag into one `ReadableChannel` with one unit. Amendment: replace every remaining `intensity.reading` reference with the two scalar descriptors, or add a separate non-scalar source type for the waveform payload.
- MAJOR - `docs/CAPABILITY_MODEL.md:Motion3D` has descriptor shape without a permanent registry identity. `Motion3D` inherits `CapabilityDescriptor`, but the seed vocabulary only publishes `stage.x/y/z`, while section 11 says those axes are "via Motion3D" and bindings drive exactly one capability. Failure case: an implementer must either invent an unratified permanent id such as `stage.position`, or never instantiate/register `Motion3D` despite the normative class tree. Amendment: make `Motion3D` a non-registered helper/envelope type, or publish a concrete `stage.position` capability with explicit axis-subdescriptor semantics.
- MAJOR - `docs/CAPABILITY_MODEL.md:section 14.2 Multi-channel HV naming` cannot be safely deferred while `TCT_app/controller/scan_controller.py:_resolve_bias` and `DeviceManager.refresh_bias_channels` already execute non-primary `BiasChannel`s. Failure case: a saved plan with `safety["bias_channel"]=1` gets serialized as `bias.voltage`/`device_id=bias_supply`, indistinguishable from the primary channel, or the capability path has no descriptor for a runnable existing plan. Amendment: choose `bias.ch{n}.voltage` (or another channel-bearing permanent id) before D1 publishes HV descriptors, or explicitly block capability provenance/execution for non-primary bias channels until the naming is ratified.
- MAJOR - `docs/CAPABILITY_MODEL.md:section 7.2 Per-OPERATION gate routing` vs `section 7.4 Generated UI` gives two incompatible rules for EMITTING setters. The route table says `wavegen.*` `SET` is ungated to match `_apply_wavegen_settings`; the generated-UI law says any operation on `safety_class >= MOTION` routes through existing QWidget gates. Failure case: the generated capability panel either prompts for every frequency/duty edit (operator will demand auto-confirm) or bypasses section 7.4. Amendment: make section 7.4 operation-aware, or assign an explicit route for EMITTING `SET` and test the generated UI contract against it.
